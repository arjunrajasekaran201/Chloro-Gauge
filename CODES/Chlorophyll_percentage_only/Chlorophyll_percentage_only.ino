...
uint16_t ired, iblue, iir;
uint16_t iredMax, iblueMax, iirMax;
float transRed, transBlue, transIR, leafIndex,leafperct;
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("Initializing Leaf Chlorophyll Index Measurement...");
  ...
  Serial.print("Red Reference (Raw): "); Serial.println(iredMax);

  Serial.println("Calibrating BLUE LED...");
  iblueMax = measureLED(BLUE_LED);
  Serial.print("Blue Reference (Raw): "); Serial.println(iblueMax);

  Serial.println("Calibrating IR LED...");
  iirMax = measureLED(IR_LED);
  Serial.print("IR Reference (Raw): "); Serial.println(iirMax);
...
void loop() {
  ...
  iblue = measureLED(BLUE_LED);
  iir = measureLED(IR_LED);

  // Avoid division by zero
  if (iredMax == 0 || iblueMax == 0 || iirMax == 0) {
  ...
  transRed = (float)ired / iredMax;
  transBlue = (float)iblue / iblueMax;
  transIR = (float)iir / iirMax;

  // Compute combined Leaf Chlorophyll Index
  leafIndex = transIR / (transRed + transBlue);
  leafperct = 100-leafIndex;
  // Print results
  Serial.print("Red: "); Serial.print(ired);
  Serial.print(" | Blue: "); Serial.print(iblue);
  Serial.print(" | IR: "); Serial.print(iir);
  Serial.print(" | Chlorophyll Percentage: "); 
  Serial.print(leafperct, 1);
  Serial.println(" %");
